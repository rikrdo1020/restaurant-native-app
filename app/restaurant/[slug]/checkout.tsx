...
export default function CheckoutScreen() {
  ...
  useEffect(() => {
    ...
    if (orderResult?.expires_at && !secondsLeft) {
      const diff = Math.floor(
        (new Date(orderResult.expires_at).getTime() - Date.now()) / 1000
      );
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          console.log("Order changed: ", newStatus);
  ...
  const qrValue = useMemo(() => {
    ...
    const orderId =
      orderResult.order_id ?? orderResult.orderId ?? orderResult.id;
  ...
  if (orderResult) {
    ...
    if (orderStatus === "ready") {
      return (
        <View style={styles.center}>
          <Text
            variant="headlineSmall"
            style={{ marginTop: 16, color: "#4caf50" }}
          >
    ...
    return (
      <ScrollView contentContainerStyle={styles.container}>
        ...
        {orderResult.skipped_items &&
          Array.isArray(orderResult.skipped_items) &&
          orderResult.skipped_items.length > 0 && (
            <>
        ...
        <View style={{ marginTop: 20 }}>
          <Button
            mode="contained"
            onPress={() => {
              /* puedes llevar al usuario al inicio o menú */ router.push("/");
            }}
            style={{ backgroundColor: "#ff6347" }}
          >
            Volver al Menú
      ...
      >
        <View style={{ marginBottom: 24 }}>
          ...
          <Text variant="bodyMedium" style={{ opacity: 0.7 }}>
            {itemCount} {itemCount === 1 ? "producto" : "productos"} • Total: $
            {total.toFixed(2)}
          </Text>
        </View>
        ...
        <View style={{ marginBottom: 24 }}>
          {items.map((it: any, idx: number) => (
            <View
              key={`${it.menu_item_id ?? it.id}-${idx}`}
              style={{ marginBottom: 8 }}
            >
              <Text>{it.name ?? it.title ?? it.menu_item_name}</Text>
              <Text style={{ opacity: 0.7 }}>
                {it.quantity} x ${it.price ?? it.price_snapshot ?? "?"}
              </Text>
        ...
        <View style={{ marginBottom: 16 }}>
          ...
          <Button
            mode="outlined"
            onPress={() => {
              /* opcional para abrir modal para ingresar nombre */
            }}
          >
